...
try:
    ...
    # plugins/clima/scripts -> plugins/clima -> plugins -> config
    p = os.path.join(script_dir, "../../../config/settings.json")
    if os.path.exists(p):
        import json
        with open(p, "r") as f:
            data = json.load(f)
            apis = data.get("apis", {})
            IP = apis.get("AC_IP", IP)
            DEVICE_ID = apis.get("AC_DEVICE_ID", DEVICE_ID)
except: pass
