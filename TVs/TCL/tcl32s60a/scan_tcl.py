...
def scan_ultra_fast(ip: str) -> Dict[str, str]:
    """Realiza un barrido ultra rápido de canales predefinidos específicos para TCL"""
    print(f"⚡ Iniciando escaneo TCL en {ip} (80.1 → 89.9)...")
    ...
    # Iniciar actividad de TV específica para TCL
    try:
        subprocess.run(["adb", "-s", target, "shell", "am", "start", "-n", "com.tcl.tv/.TVActivity"], # type: ignore
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(description="Escaneo de canales para TV TCL")
    parser.add_argument("--ip", help="IP de la TV (opcional)")
    args = parser.parse_args()

    print("⚡ Fina Ergen: TCL Ultra Fast Scan")
    print("=" * 40)
